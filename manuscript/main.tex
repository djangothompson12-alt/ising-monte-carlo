...
finds an approximately flat effective-exponent plateau down to $c=0.10$,
with additional slowing at the most dilute point, $c=0.06$. A periodic
...
Within the simulated time and size window, the data do not show the more
dramatic alternative: nowhere in this range does Model B's exponent jump
towards a qualitatively different value (e.g.\ $1/2$ or $1/4$, both of which appear in the
literature for phase separation with hydrodynamic flow, which this
lattice model does not have \cite{Bray1994}). These finite-window measurements
...
gives $\alpha=0.1676\pm0.0008$ as a fit standard error
...
per-spin bath entropy-flow rate,
\begin{equation}
\dot S_{\mathrm{bath}}(t) = -\frac{1}{T_f}\,\frac{\langle \Delta E\rangle}{dt},
\label{eq:entropy}
\end{equation}
which requires no simulation beyond the bookkeeping the Metropolis step
is already doing. It is not, by itself, the total stochastic entropy
...
satisfies detailed balance and is microscopically reversible in the
statistical sense (Section~2.3); the observed positive, decaying
...
quantitatively accounted for that discrepancy. Making the conserved model's coupling anisotropic produces
directionally elongated domains, a lattice-scale analogue of textured and
stress-rafted real microstructures. Sweeping the initial concentration away
from $50/50$ is consistent with the growth exponent being controlled
primarily by the conservation law rather than morphology: Model B forms an
approximately flat effective-exponent plateau down to $c=0.10$, followed by
additional slowing at $c=0.06$, while
