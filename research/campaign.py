...
def config_from_plan(plan, L, concentration, seed):
    """Create a frozen configuration, optionally using temperatures relative to Tc.

    Existing plans without ratio fields retain the engine defaults, preserving
    historical campaign behaviour. Ratios make a literature comparison
    unambiguous when the coupling values are changed.
    """
    fields = ("T_initial_over_tc", "T_final_over_tc")
    if any(field in plan for field in fields):
        tc = engine.anisotropic_critical_temperature(plan["Jx"], plan["Jy"])
        initial_ratio = plan.get("T_initial_over_tc")
        final_ratio = plan.get("T_final_over_tc")
        if ((initial_ratio is not None and initial_ratio <= 0)
                or (final_ratio is not None and final_ratio <= 0)):
            raise ValueError("Temperature ratios must be positive when specified.")
        initial = None if initial_ratio is None else float(initial_ratio) * tc
        final = None if final_ratio is None else float(final_ratio) * tc
    else:
        initial = final = None
    return engine.KawasakiConfig(
        L=L, Jx=plan["Jx"], Jy=plan["Jy"], concentration=concentration,
        n_replicas=1, max_sweeps=plan["max_sweeps"], n_time_samples=plan["time_samples"],
        eq_sweeps_initial=plan["equilibration"], seed=seed,
        T_initial=initial, T_final=final,
    )
    ...
    try:
        for rep, ci, c, li, L in jobs:
            ...
            if path.exists():
                ...
                completed += 1
                continue
            config = config_from_plan(plan, L, c, seed)
