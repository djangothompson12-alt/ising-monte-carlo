...
class KawasakiResult:
    """Directional domain-growth and bath-entropy-flow kinetics from a
    ...
    Attributes:
        ...
        entropy_production: Historical field name for the per-spin bath
            entropy-flow proxy -<dE>/T/dt, averaged over replicas. Total
            stochastic entropy production also includes system-entropy change.
...
def run_quench_kinetics(config: KawasakiConfig) -> KawasakiResult:
    ...
    domain-growth and bath-entropy-flow kinetics.
    ...
    logarithmically spaced sweep counts. L_x(t), L_y(t), and the historical
    S_dot(t)-named bath-flow proxy are then extracted per replica and averaged.

    Args:
        config: Kawasaki quench simulation parameters.

    Returns:
        A KawasakiResult with L_x(t), L_y(t), the bath-flow proxy, and their
